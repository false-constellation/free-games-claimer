...
if (cfg.width < 1280) {
  // otherwise 'Sign in' and .menu-account__user-name are hidden (but attached to DOM), see https://github.com/vogler/free-games-claimer/issues/335
  console.error(
    `Window width is set to ${cfg.width} but needs to be at least 1280 for GOG!`,
  );
  process.exit(1);
}
...
const context = await chromium.launchPersistentContext(cfg.dir.browser, {
  ...
  recordHar: cfg.record
    ? { path: `data/record/eg-${filenamify(datetime())}.har` }
...
while (attempts < maxAttempts) {
  try {
    ...
    await context.addCookies([
      {
        name: "CookieConsent",
        value:
          "{stamp:%274oR8MJL+bxVlG6g+kl2we5+suMJ+Tv7I4C5d4k+YY4vrnhCD+P23RQ==%27%2Cnecessary:true%2Cpreferences:true%2Cstatistics:true%2Cmarketing:true%2Cmethod:%27explicit%27%2Cver:1%2Cutc:1672331618201%2Cregion:%27de%27}",
        domain: "www.gog.com",
        path: "/",
      },
    ...
    while (await signIn.isVisible()) {
      ...
      if (email && password) {
        iframe
          ...
          .click()
          .catch((_) => {}); // Click 'Change account' (email from previous login is set in some cookie)
        ...
        iframe
          .locator("form[name=second_step_authentication]")
          .waitFor()
          .then(async () => {
            ...
            const otp = await prompt({
              ...
              validate: (n) =>
                n.toString().length == 4 || "The code must be 4 digits!",
            }); // can't use type: 'number' since it strips away leading zeros and codes sometimes have them
            await iframe
              .locator("#second_step_authentication_token_letter_1")
              .pressSequentially(otp.toString(), { delay: 10 });
        ...
        await notify(
          "gog: no longer signed in and not enough options set for automatic login.",
        );
        if (cfg.headless) {
    ...
    if (!(await banner.count())) {
      ...
      const match_all = text.match(
        /Claim (.*) and don't miss the|Success! (.*) was added to/,
      );
      ...
      if (status == "claimed" && !cfg.gog_newsletter) {
        ...
        await page
          .locator(
            'li:has-text("Marketing communications through Trusted Partners") label',
          )
          .uncheck();
        await page
          .locator('li:has-text("Promotions and hot deals") label')
          .uncheck();
      }
    }
